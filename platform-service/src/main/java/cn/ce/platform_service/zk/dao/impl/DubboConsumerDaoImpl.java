...
public class DubboConsumerDaoImpl extends BaseMongoDaoImpl<DubboConsumer> implements IDubboConsumerDao{
	...
	@Override
	public Page<DubboConsumer> findPage(int currentPage, int pageSize, String nodeId) {
		Criteria c = new Criteria();
		if(StringUtils.isNotBlank(nodeId)){
			c.and(DBFieldsConstants.DUBBO_NODE_ID).is(nodeId);
		}
		return super.findPage(new Page<DubboConsumer>(currentPage, 0, pageSize), new Query(c));
	}
