...
public class GatewayManageService {
	...
	Logger LOGGER = LoggerFactory.getLogger(GatewayManageService.class);
	/**
	 * 添加网关集群
	 * @param colEntity
	 * @return
...
//		}while(isExits.size() >0);
	...
	public Result<GatewayColonyEntity> modifyGatewayCol(GatewayColonyEntity colEntity) {
		...
		//判断参数是否可用
		if(StringUtils.isNotBlank(colEntity.getColId())){
		...
		try {
			...
			result.setErrorMessage("集群修改失败");
			LOGGER.error("modifyGatewayCol error! e:" + e.toString());
		}
		return result;
	}
	...
	public Result<String> addGatewayNode(GatewayNodeEntity nodeEntity) {
		...
		if(StringUtils.isNotBlank(nodeEntity.getColId())){
