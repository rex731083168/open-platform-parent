...
public abstract class AbstractBaseMongoDao<T> implements BaseMongoDao<T> {
	...
	/**
	 * @Description: 根据多个属性查找数据 @return Page<T> 返回类型 @throws
	 */
	public Page<T> findByFieldsAsPage(Map<String, Object> fieldAndValues, int currentPage, int pageSize,
			Class<?> entityclass) {
		Criteria criteria = this.getAndCriteria(fieldAndValues);
		/*
		 * List<Criteria> criteriaLst = new ArrayList<Criteria>(); Iterator<String> ite
		 * = fieldAndValues.keySet().iterator(); while (ite.hasNext()) { String field =
		 * ite.next(); Object val = fieldAndValues.get(field); if (criteria == null) {
		 * criteria = Criteria.where(field).is(val); } else {
		 * criteriaLst.add(Criteria.where(field).is(val)); } } if (criteria != null &&
		 * !criteriaLst.isEmpty()) { Criteria[] criterias = new
		 * Criteria[criteriaLst.size()]; for (int i = 0; i < criteriaLst.size(); i++) {
		 * criterias[i] = criteriaLst.get(i); } criteria.andOperator(criterias); }
		 */
		if (criteria == null) {
	...
	public Page<T> findLikeAsPage(String field, String value, Integer sortType, String sortField, int currentPage,
	...
	public Page<T> findLikeAndIsAsPage(String field, String value, String isField, Object isVal, Integer sortType,
			String sortField, int currentPage, int pageSize, Class<?> entityclass) {
	...
	public Page<T> findLikeOrLikeAsPage(String field, String value, String secondField, String secondVal,
		...
		Query query = new Query();
		query.addCriteria(
				c.orOperator(Criteria.where(field).regex(valuePat), Criteria.where(secondField).regex(valuePat)));
	...
	public Page<T> findLikeAndNotInAsPage(String field, String value, String ninField, Collection<?> ninVal,
			Integer sortType, String sortField, int currentPage, int pageSize, Class<?> entityclass) {
		Criteria c = null;
		if (ninVal != null) {
	...
	public Page<T> findEqAndInAsPage(Map<String, Object> eqFields, Map<String, Collection<?>> inFields, int currentPage,
			int pageSize, Class<?> entityclass) {
	...
	public List findEqAndInField(Map<String, Object> eqFields, Map<String, Collection<?>> inFields,
			Class<?> entityClass) {
		Criteria c = this.getIsCriteria(eqFields);
		c = this.getInCriteria(c, inFields);
		if (c == null) {
	...
	public List findEqAndNotInField(Map<String, Object> eqFields, Map<String, Collection<?>> inFields,
			Class<?> entityClass) {
		Criteria c = this.getIsCriteria(eqFields);
	...
	public T findOneByFieldsAnd(Map<String, Object> fields, Class<?> entityclass) {
		...
		/*
		 * Iterator<String> ite = fields.keySet().iterator(); while (ite.hasNext()) {
		 * String field = ite.next(); String val = fields.get(field); if (a == null) { a
		 * = Criteria.where(field).is(val); } else {
		 * a.andOperator(Criteria.where(field).is(val)); } }
		 */
	...
	@Override
	public Page<T> findPageListByWhereEntity(int currentPage, int pageSize, Map<String, MongoDBWhereEntity> whereMap,
			Class<?> entityclass) {
		Query query = new Query();
		Criteria criteriaByWhereEntity = getCriteriaByWhereEntity(whereMap);
		query.addCriteria(criteriaByWhereEntity);
